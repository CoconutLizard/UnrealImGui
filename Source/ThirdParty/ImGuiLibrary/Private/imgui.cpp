...
void ImGui::RenderText(ImVec2 pos, const char* text, const char* text_end, bool hide_text_after_hash, const bool bHighlight)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    if (text_len > 0)
    {
        window->DrawList->AddText(g.Font, g.FontSize, pos, GetColorU32(!bHighlight ? ImGuiCol_Text : ImGuiCol_Highlight), text, text_display_end);
        if (g.LogEnabled)
...
void ImGui::RenderTextClipped(const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect, const bool bHighlight)
{
    // Hide anything after a '##' string
    ...
    if (need_clipping)
    {
        ...
        window->DrawList->AddText(g.Font, g.FontSize, pos, GetColorU32(!bHighlight ? ImGuiCol_Text : ImGuiCol_Highlight), text, text_display_end, 0.0f, &fine_clip_rect);
    }
    else
    {
        window->DrawList->AddText(g.Font, g.FontSize, pos, GetColorU32(!bHighlight ? ImGuiCol_Text : ImGuiCol_Highlight), text, text_display_end, 0.0f, NULL);
    }
    if (g.LogEnabled)
        LogRenderedText(&pos, text, text_display_end);
}
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (display_frame)
    {
        ...
        if (g.LogEnabled)
        {
            ...
            RenderTextClipped(text_pos, bb.Max, label, label_end, &label_size, ImVec2(0, 0), NULL, (flags & ImGuiTreeNodeFlags_Highlight) != 0);
    ...
    else
    {
        ...
        RenderText(text_pos, label, label_end, false, (flags & ImGuiTreeNodeFlags_Highlight) != 0);
    }
...
bool ImGui::DragBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_speed, float v_min, float v_max, int decimal_precision, float power, bool bDown, bool bDrawFrame)
{
    ...
    if (g.ActiveId == id)
    {
        if (g.IO.MouseDown[0])
        {
            ...
            {
                adjust_delta = !bDown ? mouse_drag_delta.x - g.DragLastMouseDelta.x : mouse_drag_delta.y - g.DragLastMouseDelta.y;
            ...
            g.DragLastMouseDelta = mouse_drag_delta;
...
IMGUI_API bool ImGui::DragFloatButton(const char * label, float * v, float v_speed, float v_min, float v_max, const char * display_format, float power, bool bDown)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImVec2 max_button_position = ImVec2(w, label_size.y + style.FramePadding.y*2.0f);
	const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y*2.0f));
	const ImRect inner_bb(frame_bb.Min + style.FramePadding, frame_bb.Max - style.FramePadding);
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));

	//const ImVec2 corner = ImLerp(window->Pos, window->Pos + window->Size, grip.CornerPos);
	/*window->DrawList->PathLineTo(frame_bb.Min + max_button_position);
	window->DrawList->PathLineTo(frame_bb.Min);
	window->DrawList->PathArcToFast(frame_bb.Min + max_button_position, 10.0f, 10.0f, 10.0f);*/
	//window->DrawList->PathFillConvex(frame_bb);

	ImGui::RenderTriangle(frame_bb.Min, !bDown ? ImGuiDir_Right : ImGuiDir_Down, 1.0f);
	...
	ItemSize(total_bb, style.FramePadding.y);
	const bool value_changed = DragBehavior(frame_bb, id, v, v_speed, v_min, v_max, decimal_precision, power, bDown);
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data, bool* bIsClicked)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
	...
	if (bIsClicked)
	{
		*bIsClicked |= user_clicked;
		*bIsClicked &= !(!hovered && io.MouseClicked[0]);
	}
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data, bool* bIsClicked)
{
    IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
    return InputTextEx(label, buf, (int)buf_size, ImVec2(0,0), flags, callback, user_data, bIsClicked);
}
...
ImGui::ImGuiUserStyle::ImGuiUserStyle() {
	Colors[ImGuiUserCol_TabNormal] = ImVec4(0.35f, 0.40f, 0.61f, 0.62f);
	Colors[ImGuiUserCol_TabTitleTextNormal] = ImVec4(1.0f, 1.0f, 1.0f, 1.0f);
	Colors[ImGuiUserCol_TabTitleTextSelected] = ImVec4(1.0f, 1.0f, 1.0f, 1.0f);
	...
	Colors[ImGuiUserCol_TabHover] = ImVec4(0.40f, 0.48f, 0.71f, 0.79f);
}

const ImU32 ImGui::GetColorU32(ImGuiUserCol_ idx, float alpha_mul) {
	ImVec4 c = GetUserStyle().Colors[idx];
	if (alpha_mul > 0.0f)
...
void ImGui::TabBar::_drawTabBarTop(const char *label) {
	...
	const float tab_height = CalcTextSize(tabTitles[0]).y + (frame_padding.y * 4);
	...
	for (int i = 0; i < tabCount; i++) {
		...
		if (!selected) {
			...
			const ImVec2 text_pos = pos + ImVec2(offs + ((xr - xl) - text_size.x) / 2.0f, (tab_height * 0.5f) - (text_size.y*0.5f)/*((text_size.y - frame_padding.y*4.0f) / 2.0f)*/);
	...
	dl->AddRectFilled(bb.Min, bb.Max, ImColor(0.40f, 0.48f, 0.71f, 0.79f), rounding, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_TopRight);
	// Draw the border
	_drawPartialRect(bb.Min, bb.Max, rounding, ImDrawCornerFlags_TopLeft | ImDrawCornerFlags_TopRight, dl,
		(_EdgeType)(EDGE_LEFT | EDGE_RIGHT | EDGE_TOP), GetColorU32(ImGuiUserCol_TabBorderShadow),
	...
	const ImVec2 text_pos = pos + ImVec2(selected_offset + ((xr - xl) - text_size.x) / 2.0f - selected_expands.x, (tab_height * 0.5f) - (text_size.y*0.5f));
...
void ImGui::BeginTabBar(const char *label, const ImVec2 size) {
	...
	if (!GetTabBarStack().doesTabBarExist(label, &idx)) {
		TabBar bar = TabBar(label, size);
		GetTabBarStack().TabBars.push_back(bar);
		GetTabBarStack().CurrentTabBar = GetTabBarStack().TabBarCount;
		GetTabBarStack().TabBarCount++;
	}
	else { // Set the current active tab bar
		GetTabBarStack().CurrentTabBar = idx;
		GetTabBarStack().clearIdxCountCurrentTabBar();
	}
	GetTabBarStack().getCurrentTabBar()->_drawTabBarTop(label);
}

const bool ImGui::AddTab(const char *title) {
	// Get current TabBar;
	TabBar* bar = GetTabBarStack().getCurrentTabBar();
